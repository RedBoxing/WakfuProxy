package fr.redboxing.wakfu.proxy.utils.serialization;
...
public abstract class BinarSerial
{
    ...
    public final void fromBuild(final byte[] binarBuild, final int version) {
        ...
        for (int i = 0; i < tocLength; ++i) {
            ...
            if (size <= 0) {
                WakfuProxy.getInstance().getLogger().warn((Object)("Part " + index + "(offset=" + offset + ") is empty for " + this + " ! (voir log serveur)"), (Throwable)new Exception());
            }
            else if (size > 10485760) {
                WakfuProxy.getInstance().getLogger().error((Object)("Part " + index + "(offset=" + offset + ") exceeds max limit (" + size + " > " + 10485760 + " bytes)"), (Throwable)new Exception());
            }
            else {
                ...
                if (index >= 0 && index < parts.length) {
                    ...
                    if (part == BinarSerialPart.EMPTY) {
                        WakfuProxy.getInstance().getLogger().warn((Object)("Don't know how to unserialise part #" + index + " (EMPTY)."));
                        continue;
                    }
                    if (part == null) {
                        WakfuProxy.getInstance().getLogger().error((Object)("Part " + index + " of " + this + " is null"), (Throwable)new Exception());
                        continue;
                    }
                    try {
                        part.clearError();
                    ...
                    if (partBuffer.remaining() > 0) {
                        WakfuProxy.getInstance().getLogger().warn((Object)("Part " + index + " of " + this + " still have " + partBuffer.remaining() + " byte(s) left !"), (Throwable)new Exception());
                    }
                }
        ...
        for (int tocLength = buffer.get(), i = 0; i < tocLength; ++i) {
            ...
            if (tocIndex == index) {
                ...
                try {
                    ...
                    WakfuProxy.getInstance().getLogger().error((Object)("Exception lev\u00e9e lors de la d\u00e9serialisation de la part :" + index), (Throwable)e);
                }
                return;
            }
        }
